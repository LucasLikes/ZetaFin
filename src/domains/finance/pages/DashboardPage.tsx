...
// ─── SIDEBAR ─────────────────────────────────────────────────────────────────
const Sidebar = ({ page, setPage, user, onLogout, C }: {
    ...
    <aside style={{
        ...
        {NAV.map(({ id, label, icon: Ic }) => {
          ...
          return (
            <button key={id} onClick={() => setPage(id)} title={!expanded ? label : undefined} style={{
              ...
              borderRadius: '8px', width: '100%', border: 'none',
      ...
      <div style={{ padding: expanded ? '12px 10px' : '12px 6px', borderTop: `1px solid ${C.sideBdr}` }}>
        <button onClick={onLogout} title={!expanded ? 'Sair' : undefined} style={{
          ...
          width: '100%', borderRadius: '8px', border: 'none', background: 'transparent',
...
// ─── DONUT CHART ─────────────────────────────────────────────────────────────
const DonutChart = ({ transactions, C }: { transactions: Transaction[]; C: ReturnType<typeof getThemeColors> }) => {
    ...
    <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
      <div style={{ position: 'relative', flexShrink: 0 }}>
        <svg width="160" height="160" viewBox="0 0 160 160">
          ...
          <circle cx="80" cy="80" r="40" fill={C.donutCenter} />
          {hov ? (
            <>
      ...
      <div style={{ flex: 1, minWidth: '130px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
        {arcs.slice(0, 6).map(({ cat, val, pct, color }) => (
          <div key={cat} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'default', opacity: hovered && hovered !== cat ? 0.35 : 1, transition: 'opacity .2s' }}
            ...
            <div style={{ fontSize: '11px', fontWeight: 600, color: C.muted }}>{(pct * 100).toFixed(0)}%</div>
            <div style={{ fontSize: '11px', color: C.muted2 }}>{brl(val)}</div>
          </div>
        ))}
      </div>
    </div>
...
// ─── BAR CHART ───────────────────────────────────────────────────────────────
const BarChart = ({ C }: { C: ReturnType<typeof getThemeColors> }) => {
    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '10px', height: '100%', paddingBottom: '22px' }}>
      {months.map((m, i) => (
        <div key={m} style={{ flex: 1, display: 'flex', gap: '4px', alignItems: 'flex-end', position: 'relative' }}>
          <div title={`Despesas: ${brl(des[i])}`} style={{ flex: 1, height: `${Math.round((des[i] / max) * 110)}px`, background: `${C.red}88`, borderRadius: '6px 6px 0 0', transition: 'opacity .2s', cursor: 'default' }}
            onMouseEnter={e => (e.currentTarget.style.opacity = '1')} onMouseLeave={e => (e.currentTarget.style.opacity = '0.55')} />
          <div title={`Entradas: ${brl(ent[i])}`} style={{ flex: 1, height: `${Math.round((ent[i] / max) * 110)}px`, background: `${C.green}aa`, borderRadius: '6px 6px 0 0', transition: 'opacity .2s', cursor: 'default' }}
...
// ─── METRIC CARD ─────────────────────────────────────────────────────────────
const MetricCard = ({ label, value, valueSub, color, sub, accentBar, C }: {
  ...
  >
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: accentBar, opacity: 0.8 }} />
...
// ─── TX ROW ──────────────────────────────────────────────────────────────────
const TxRow = ({ tx, onDelete, C }: { tx: Transaction; onDelete?: (id: string) => void; C: ReturnType<typeof getThemeColors> }) => {
...
const TxModal = ({ open, onClose, onSave, C }: {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', backdropFilter: 'blur(4px)' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '16px', width: '100%', maxWidth: '420px', padding: '24px', boxShadow: C.shadowMd, fontFamily: FONT }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div>
            ...
            <input value={desc} onChange={e => setDesc(e.target.value)} placeholder="Ex: Mercado, Salário..." style={inputStyle}
              onFocus={e => { (e.target as HTMLInputElement).style.borderColor = C.primary; (e.target as HTMLInputElement).style.boxShadow = `0 0 0 3px ${C.greenDim}` }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div>
              ...
              <input type="number" min="0" step="0.01" value={valor} onChange={e => setValor(e.target.value)} placeholder="0,00" style={inputStyle}
                onFocus={e => { (e.target as HTMLInputElement).style.borderColor = C.primary; (e.target as HTMLInputElement).style.boxShadow = `0 0 0 3px ${C.greenDim}` }}
                onBlur={e => { (e.target as HTMLInputElement).style.borderColor = C.border; (e.target as HTMLInputElement).style.boxShadow = 'none' }} />
            </div>
            <div>
              ...
              <select value={cat} onChange={e => setCat(e.target.value)} style={{ ...inputStyle, appearance: 'none' } as React.CSSProperties}
                onFocus={e => { (e.target as HTMLSelectElement).style.borderColor = C.primary; (e.target as HTMLSelectElement).style.boxShadow = `0 0 0 3px ${C.greenDim}` }}
        ...
        <div style={{ display: 'flex', gap: '8px', marginTop: '20px' }}>
          ...
          <button onClick={save} disabled={saving || !desc || Number(valor) <= 0} style={{
            ...
            background: saving || !desc || Number(valor) <= 0 ? C.greenDim : C.primary,
...
// ─── PAGE INICIO ─────────────────────────────────────────────────────────────
const PageInicio = ({ finance, onAddTx, onGoPage, C }: {
  finance: ReturnType<typeof useFinance>; onAddTx: () => void
  onGoPage: (p: Page) => void; C: ReturnType<typeof getThemeColors>
}) => {
  const savingsGoals: Goal[] = [
    {
      id: '1', title: 'Reserva de emergência', description: 'Meu fundo para imprevistos',
      targetAmount: 5000, currentAmount: 1200, type: 'investment',
      yieldRate: 0.115, monthlyContribution: 200, createdAt: new Date().toISOString(),
    },
  ]
  ...
  return (
    <div>
      <div style={{ marginBottom: '20px' }}>
        {savingsGoals.map(goal => <SavingsBox key={goal.id} goal={goal} />)}
      </div>

      {/* gráfico semanal dark */}
      ...
      <div className="metrics-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '18px' }}>
        {finance.loading
          ...
          : <>
            <MetricCard C={C} label="Saldo do mês"  value={brl(finance.summary?.saldo ?? 0)}       color={C.text}  accentBar={C.borderMid} sub="entradas − despesas" />
            <MetricCard C={C} label="Entradas"       value={brl(finance.summary?.entradas ?? 0)}    valueSub="+5.2%" color={C.green} accentBar={C.green} sub="mês atual" />
            <MetricCard C={C} label="Despesas"       value={brl(finance.summary?.despesas ?? 0)}    color={C.red}   accentBar={C.red}   sub="+8% vs anterior" />
            <MetricCard C={C} label="Limite diário"  value={brl(finance.summary?.limiteDiario ?? 0)} color={C.blue} accentBar={C.blue}  sub="Baseado no seu salário" />
          </>
        }
      </div>
      ...
      <div className="row2-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '18px 20px', boxShadow: C.shadow }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            <span style={{ fontSize: '13px', fontWeight: 700, color: C.text, fontFamily: FONT }}>Entradas vs Despesas</span>
            <div style={{ display: 'flex', gap: '12px' }}>
              {([[C.red, 'Despesas'], [C.green, 'Entradas']] as [string, string][]).map(([c, l]) => (
                <div key={l} style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '11px', color: C.muted, fontFamily: FONT }}>
                  <div style={{ width: '7px', height: '7px', borderRadius: '2px', background: c }} />{l}
                </div>
              ))}
            </div>
          </div>
          <div style={{ height: '148px' }}><BarChart C={C} /></div>
        </div>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '18px 20px', boxShadow: C.shadow }}>
          <div style={{ fontSize: '13px', fontWeight: 700, color: C.text, marginBottom: '14px', fontFamily: FONT }}>Gastos por categoria</div>
          <DonutChart transactions={finance.transactions} C={C} />
        </div>
      </div>
      ...
      <div className="row3-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '14px' }}>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '18px 20px', boxShadow: C.shadow }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            <span style={{ fontSize: '13px', fontWeight: 700, color: C.text, fontFamily: FONT }}>Últimas transações</span>
          ...
          {finance.transactions.slice(0, 6).map(tx => <TxRow key={tx.id} tx={tx} C={C} />)}
        </div>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '18px 20px', boxShadow: C.shadow }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            <span style={{ fontSize: '13px', fontWeight: 700, color: C.text, fontFamily: FONT }}>Dívidas ativas</span>
          ...
          {finance.debts.slice(0, 3).map(d => {
            ...
            return (
              <div key={d.id} style={{ marginBottom: '13px' }}>
                ...
                <div style={{ background: C.surf2, borderRadius: '3px', height: '4px', overflow: 'hidden' }}>
...
const PageHistorico = ({ finance, C }: { finance: ReturnType<typeof useFinance>; C: ReturnType<typeof getThemeColors> }) => {
  const now = new Date()
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', flexWrap: 'wrap' }}>
        {[
          ...
          <div key={label} style={{ padding: '9px 16px', borderRadius: '9px', background: bg, border: `1px solid ${bd}`, display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: '14px', fontWeight: 700, color, fontFamily: FONT }}>{brl(Math.abs(val))}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '18px 20px', boxShadow: C.shadow }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
          <span style={{ fontSize: '13px', fontWeight: 700, color: C.text, fontFamily: FONT }}>
            ...
            {(['all', 'entrada', 'despesa'] as const).map(f => {
              ...
              return (
                <button key={f} onClick={() => setFilter(f)} style={{
                  ...
                  background: active ? `${ac}15` : 'none',
                  color: active ? ac : C.muted,
                  border: `1px solid ${active ? `${ac}40` : C.border}`,
                }}>{labels[f]}</button>
              )
            })}
          </div>
        ...
        <div style={{ position: 'relative', marginBottom: '12px' }}>
          ...
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar transação ou categoria..." style={{ ...inputStyle, paddingLeft: '38px' }}
            onFocus={e => { (e.target as HTMLInputElement).style.borderColor = C.primary; (e.target as HTMLInputElement).style.boxShadow = `0 0 0 3px ${C.greenDim}` }}
            onBlur={e => { (e.target as HTMLInputElement).style.borderColor = C.border; (e.target as HTMLInputElement).style.boxShadow = 'none' }} />
        </div>
        {finance.loading
          ...
          : filtered.length === 0
            ...
            : filtered.map(tx => <TxRow key={tx.id} tx={tx} onDelete={finance.deleteTransaction} C={C} />)
        }
      </div>
    </div>
  )
...
// ─── PAGE DIVIDAS ─────────────────────────────────────────────────────────────
const PageDividas = ({ finance, C }: { finance: ReturnType<typeof useFinance>; C: ReturnType<typeof getThemeColors> }) => {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '14px', alignItems: 'start' }}>
              ...
              return (
                <div key={d.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '16px 18px', marginBottom: '10px', boxShadow: C.shadow }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                    <div>
                      <div style={{ fontSize: '14px', fontWeight: 700, color: C.text, fontFamily: FONT }}>
                        {d.nome}{' '}
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 9px', borderRadius: '20px', background: `${clr}15`, color: clr, border: `1px solid ${clr}30`, fontFamily: FONT }}>{pct.toFixed(0)}%</span>
                  ...
                  <div style={{ background: C.surf2, borderRadius: '3px', height: '5px', overflow: 'hidden', marginBottom: '8px' }}>
                    <div style={{ height: '100%', borderRadius: '3px', background: clr, width: `${pct.toFixed(1)}%`, transition: 'width .4s' }} />
                  </div>
                  {!quitada && (
                    <div style={{ display: 'flex', gap: '7px', marginTop: '10px' }}>
                      <input type="number" min="0" step="0.01" placeholder="Valor do pagamento" value={pags[d.id] || ''} onChange={e => setPags(p => ({ ...p, [d.id]: e.target.value }))} style={{ ...inputStyle, flex: 1 }}
                        onFocus={e => { (e.target as HTMLInputElement).style.borderColor = C.amber; (e.target as HTMLInputElement).style.boxShadow = `0 0 0 3px ${C.amberDim}` }}
                        onBlur={e => { (e.target as HTMLInputElement).style.borderColor = C.border; (e.target as HTMLInputElement).style.boxShadow = 'none' }} />
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '16px 18px', position: 'sticky', top: '70px', boxShadow: C.shadow }}>
        <div style={{ fontSize: '13px', fontWeight: 700, color: C.text, marginBottom: '12px', fontFamily: FONT }}>Nova dívida</div>
...
// ─── PAGE METAS ───────────────────────────────────────────────────────────────
const PageMetas = ({ C }: { C: ReturnType<typeof getThemeColors> }) => {
  const goals = [
          ...
          return (
            <div key={g.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '18px 20px', boxShadow: C.shadow }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                <div style={{ fontSize: '14px', fontWeight: 700, color: C.text, fontFamily: FONT }}>{g.title}</div>
                <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 9px', borderRadius: '20px', background: `${g.cor}15`, color: g.cor, border: `1px solid ${g.cor}30`, fontFamily: FONT }}>{pct.toFixed(0)}%</span>
              </div>
              <div style={{ background: C.surf2, borderRadius: '5px', height: '6px', overflow: 'hidden', marginBottom: '10px' }}>
...
// ─── PAGE CONFIG ──────────────────────────────────────────────────────────────
const PageConfig = ({ C }: { C: ReturnType<typeof getThemeColors> }) => {
  ...
  const statValue: React.CSSProperties = { fontSize: '20px', fontWeight: 700, color: C.text }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '16px', maxWidth: '1000px' }}>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: '16px', padding: '18px', boxShadow: C.shadow }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <input value={name} onChange={e => setName(e.target.value)} style={{ ...inputStyle, textAlign: 'center', marginBottom: '6px' }} />
          <div style={{ fontSize: '12px', color: C.muted }}>{email}</div>
          <button style={{ marginTop: '12px', width: '100%', padding: '10px', borderRadius: '8px', border: 'none', background: C.primary, color: C.primaryText, fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}>
            Editar Perfil
          </button>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
        ...
        <div style={{ ...cardStat, gridColumn: 'span 3' }}>
          <div style={statTitle}>Tema da interface</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '8px' }}>
            <span style={{ fontSize: '13px', color: C.muted, fontFamily: FONT }}>{isDark ? 'Modo escuro ativo' : 'Modo claro ativo'}</span>
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: ${C.bg}; color: ${C.text}; transition: background .2s, color .2s; }
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, fontFamily: FONT, color: C.text, fontSize: '14px', transition: 'background .2s' }}>
        <div className="sidebar-wrap" style={{ display: 'flex', flexShrink: 0 }}>
          <Sidebar page={page} setPage={setPage} user={user} onLogout={handleLogout} C={C} />
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <nav style={{
            ...
            position: 'sticky', top: 0, zIndex: 50, flexShrink: 0, gap: '14px',
                ...
                {menuOpen && (
                  <div onClick={(e) => e.stopPropagation()} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px' }}>
                      <div style={{ width: '36px', height: '36px', borderRadius: '20%', background: C.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: C.primaryText }}>
                        {user?.name?.charAt(0).toUpperCase()}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '13px', fontWeight: 600, color: '#e5e7eb' }}>
                          {user?.name}
                          <span style={{ marginLeft: '6px', fontSize: '10px', background: C.primary, padding: '2px 6px', borderRadius: '6px', color: C.primaryText }}>PRO</span>
                        </div>
                    ...
                    {[
                      { label: 'Configurações', action: () => setPage('config') },
                      { label: isDark ? 'Modo claro' : 'Modo escuro', action: toggleTheme },
                      { label: 'Help Center', action: () => {} },
                    ].map((item, i) => (
                      <button key={i} onClick={() => { item.action(); setMenuOpen(false) }} style={{ width: '100%', padding: '10px 14px', background: 'transparent', border: 'none', textAlign: 'left', fontSize: '13px', color: '#e5e7eb', cursor: 'pointer', fontFamily: FONT }}
                        onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.05)')}
                        onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                      >{item.label}</button>
                    ))}
                    <div style={{ height: '1px', background: 'rgba(255,255,255,0.06)' }} />
                    <button onClick={() => { handleLogout(); setMenuOpen(false) }} style={{ width: '100%', padding: '10px 14px', background: 'transparent', border: 'none', textAlign: 'left', fontSize: '13px', color: '#f87171', cursor: 'pointer', fontFamily: FONT }}
                      onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.05)')}
                      onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                    >Sair</button>
                  </div>
          ...
          <main className="content-main" style={{ flex: 1, overflowY: 'auto', padding: '22px 24px', background: C.bg, transition: 'background .2s' }}>
            <div key={page} className="page-anim">
              {page === 'inicio'    && <PageInicio   finance={finance} onAddTx={() => setModal(true)} onGoPage={setPage} C={C} />}
              {page === 'historico' && <PageHistorico finance={finance} C={C} />}
              {page === 'dividas'   && <PageDividas  finance={finance} C={C} />}
              {page === 'metas'     && <PageMetas    C={C} />}
              {page === 'config'    && <PageConfig   C={C} />}
            </div>
          </main>
        </div>

        {/* bottom nav mobile */}
        <nav className="bnav" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, width: '100%', background: C.navBg, borderTop: `1px solid ${C.navBorder}`, zIndex: 100, height: '64px', display: 'flex', justifyContent: 'space-around', transition: 'background .2s' }}>
          {NAV.map(({ id, label, icon: Ic }) => (
            <button key={id} onClick={() => setPage(id)} style={{ flex: 1, maxWidth: '20%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '4px', background: 'none', border: 'none', color: page === id ? C.primary : C.muted2, fontSize: '10px', fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}>
              <Ic />
      ...
      <TxModal open={modal} onClose={() => setModal(false)} onSave={handleSaveTx} C={C} />
    </>
  )
}
